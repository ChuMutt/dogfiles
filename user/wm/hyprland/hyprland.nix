...
in {
      ...
      + ''
        ...
        plugin {
          ...
          55)
              ...
              bind=SUPERSHIFT,RETURN,exec,'' + userSettings.term + " " + ''
                --class float_term
                  ...
                  bind=SUPER,S,exec,'' + userSettings.spawnBrowser + ''
                    ...
                    bind=,code:148,exec,'' + userSettings.term + " " + ''
                      -e numbat
  ...
  home.packages = (with pkgs; [
    ...
    '')
  ]) ++ (with pkgs-hyprland; [ ]) ++ (with pkgs-nwg-dock-hyprland;
    [
  ...
  programs.waybar = {
    ...
    package = pkgs.waybar.overrideAttrs (oldAttrs: {
      ...
      '';
      patches =
        [ ./patches/waybarpaupdate.patch ./patches/waybarbatupdate.patch ];
    });
    settings = {
      mainBar = {
        layer = "top";
  ...
  programs.fuzzel.package = pkgs.fuzzel.overrideAttrs
    (oldAttrs: { patches = ./patches/fuzzelmouseinput.patch; });
