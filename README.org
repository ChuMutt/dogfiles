...
Note: Additionally, this script is executed whenever the =install= script is executed. This script can be undone by running its inverse, =soften=, and vice versa.
...
#+begin_src sh :noeval yes :shebang #!/bin/sh :tangle ./bin/install-unstable
nix-shell -p git --command "nix run --experimental-features 'nix-command flakes' github:chumutt/dogfiles -- ~/.config/dogfiles"
#+end_src
* References / Credits
